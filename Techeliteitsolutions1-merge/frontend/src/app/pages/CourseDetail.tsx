...
export function CourseDetail() {
  ...
  if (!course) {
    return (
      <div className="min-h-screen bg-white dark:bg-slate-900 pt-24 pb-16 flex items-center justify-center">
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 pt-24 pb-16 flex items-center justify-center">
        <motion.div
          ...
          className="text-center bg-white dark:bg-slate-900 rounded-2xl p-12 shadow-xl max-w-md"
        >
          <div className="w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle className="w-10 h-10 text-blue-600" />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Enrollment Successful!</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              className="bg-white dark:bg-slate-900 border-2 border-gray-100 dark:border-slate-800 rounded-xl p-8 mb-8">
              ...
              <div className="space-y-4">
                {course.curriculum.map((module, index) => (
                  <div key={index} className="border-l-4 border-blue-600 pl-6 py-2">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{module.module}</h3>
                    <ul className="space-y-2">
                      {module.topics.map((topic, idx) => (
                        <li key={idx} className="text-gray-600 dark:text-gray-300 flex items-center gap-2">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
              className="bg-white dark:bg-slate-900 border-2 border-gray-100 dark:border-slate-800 rounded-xl p-8 mb-8">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Course Features</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {course.features.map((feature, index) => (
                  <div key={index} className="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/40 rounded-lg p-4">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}
              className="bg-white dark:bg-slate-900 border-2 border-gray-100 dark:border-slate-800 rounded-xl p-8">
          ...
          <div className="lg:col-span-1">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              ...
              {!showEnrollForm ? (
                <div className="bg-white dark:bg-slate-900 border-2 border-gray-200 dark:border-slate-700 rounded-2xl p-8 shadow-xl">
                  <div className="text-center mb-6">
                    <div className="text-5xl font-bold text-blue-600 mb-2">{course.price}</div>
                    <div className="text-gray-600 dark:text-gray-300">One-time payment</div>
                  </div>
                  ...
                  <div className="border-t pt-6 space-y-4">
                    ...
                    <div className="space-y-3">
                      {course.features.map((feature, index) => (
                        <div key={index} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-200">
                ...
                <div className="bg-white dark:bg-slate-900 border-2 border-gray-200 dark:border-slate-700 rounded-2xl p-8 shadow-xl">
                  <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Complete Enrollment</h3>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      ...
                      <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required
                        className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all dark:bg-slate-800 dark:text-white"
                    ...
                    <div>
                      <label htmlFor="phone" className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Phone *</label>
                    ...
                    <div>
                      <label htmlFor="date" className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Start Date *</label>
                    ...
                    <div>
                      ...
                      <textarea id="message" name="message" value={formData.message} onChange={handleChange} rows={3}
                        className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all dark:bg-slate-800 dark:text-white resize-none"
                        placeholder="Any questions or special requirements..." />
                    </div>

                    <div className="bg-blue-50 dark:bg-blue-900/40 rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold text-gray-900 dark:text-white">Course Fee:</span>
                        <span className="text-2xl font-bold text-blue-600">{course.price}</span>
                      </div>
                      <p className="text-xs text-gray-600 dark:text-gray-300">Price for {course.title}</p>
                    </div>
                    ...
                    <button type="button" onClick={() => setShowEnrollForm(false)}
                      className="w-full bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-200 py-3 rounded-lg font-semibold hover:bg-gray-200 transition-all">
                      Back
                    </button>
                  </form>
                </div>
