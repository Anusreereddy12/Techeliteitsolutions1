    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-800/50 pb-16">
      <div className="max-w-7xl mx-auto px-6 pt-8">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Student Enrollments</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-2">Manage and review all student enrollments</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {(Object.entries(counts) as [string, number][]).map(([s, c]) => (
            <button key={s} onClick={() => setFilter(s as any)}
              className={`p-4 rounded-xl border-2 transition-all ${filter === s ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/40' : 'border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-blue-200'}`}
            >
              <div className="text-2xl font-bold text-gray-900 dark:text-white">{c}</div>
              <div className="text-sm text-gray-600 dark:text-gray-300 capitalize">{s}</div>
            </button>
          ))}
        </div>

        <div className="bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-slate-800 mb-6">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
            <input type="text" placeholder="Search by name, email, or course..."
              value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 focus:border-blue-500 outline-none"
            />
          </div>
        </div>
                ...
                <thead className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-700">
                  <tr>
                    {['Student', 'Course', 'Date', 'Amount', 'Status', 'Actions'].map(h => (
                      <th key={h} className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {filtered.length === 0 ? (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{e.course}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300"><Calendar size={13} />{e.date}</div>
                      </td>
                      <td className="px-6 py-4 font-semibold text-gray-900 dark:text-white">{e.amount || '—'}</td>
                      <td className="px-6 py-4">
