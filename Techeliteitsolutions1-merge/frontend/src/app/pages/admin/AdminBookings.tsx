    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
          ...
          <button key={s} onClick={() => setFilter(s)}
            className={`px-4 py-2 rounded-xl text-sm font-semibold border transition-all ${
              filter === s
                ...
                : 'bg-white dark:bg-slate-900 text-slate-600 border-slate-200 hover:border-blue-200'
            }`}
          >
            <span className="capitalize">{s}</span>
            <span className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${filter === s ? 'bg-white dark:bg-slate-900/20 text-white' : 'bg-slate-100 text-slate-500'}`}>{c}</span>
          </button>
        ))}
      </div>
      ...
      <div className="relative mb-6">
        ...
        <input type="text" placeholder="Search by name, email or course…"
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all"
        />
      </div>
        ...
        <div className="space-y-3">
          {filtered.map((b, i) => (
            <motion.div key={b.id}
              ...
              className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition-shadow"
            >
